...
%     fig=figure(k); %Em commented out to try another style of figure
    ...
    ax = gca(fig);
%     ax.ColorOrder=[0 0 0; 0.5 0.5 0.5; 0.3 0.3 0.3; 0.7 0.7 0.7]; %WT r5
...
%     ax.ColorOrder=[0.757 0.867 0.776; 0.35 0.4 0.35; 0.522 0.74 0.549];%S421D lyso
    ...
    hold on
%     cellfun(@(X,Y) plot(ax, X, Y), xk,yk)
%     Z=tim;
%     cellfun(@(X,Y) patch([X nan],[Y nan],[Z nan], 'edgecolor',
%     'interp'),xk,yk); %Emily trying some nonsense
%     colorbar;colormap(jet);
    cellfun(@(X,Y) plot(ax, X, Y), tim, pos_tim)
